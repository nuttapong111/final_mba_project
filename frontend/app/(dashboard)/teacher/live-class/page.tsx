...
export default function TeacherLiveClassPage() {
  ...
  const allSessions: LiveSession[] = myCourses
    ...
    .flatMap((course) =>
      (course.liveSessions || []).map((session) => ({
        ...
        enrolledStudents: course.enrolledStudents?.length || course.students,
      }))
    );
  ...
  const handleStartClass = (session: LiveSession) => {
    Swal.fire({
      title: 'เริ่มห้องเรียนออนไลน์',
      html: `
        <div class="text-left">
      ...
      `,
      icon: 'info',
      confirmButtonText: 'เปิดห้องเรียน',
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">จัดการการสอนสดตามวันเวลาที่กำหนด</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">สอนเสร็จแล้ว</p>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>{session.enrolledStudents} คน</span>
                  </div>
                </div>
                ...
                {session.meetingLink && (
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-sm font-mono text-gray-900 break-all">{session.meetingLink}</p>
                    {session.meetingPassword && (
                      <>
                        <p className="text-xs text-gray-600 mt-2 mb-1">Password:</p>
                        <p className="text-sm font-mono text-gray-900">{session.meetingPassword}</p>
                      </>
                    )}
                  </div>
                )}
                ...
                <div className="flex space-x-2 pt-2 border-t border-gray-200">
                  ...
                  {session.status === 'live' && (
                    <Button
                      ...
                      className="flex-1"
                    >
                      <VideoCameraIcon className="h-4 w-4 mr-2 inline" />
                      เข้าห้องเรียน
                    </Button>
                  )}
